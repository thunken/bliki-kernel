		...
		if (nodes != null && !nodes.isEmpty()) {
			for (Object item : nodes) {
				if (item != null && (item instanceof TagNode)) {
					...
					if (tagNode.getName().equalsIgnoreCase(INPUT_TAG)
							|| tagNode.getName().equalsIgnoreCase(TEXTAREA_TAG)) {
						...
						for (Map.Entry<String, String> entry : eSet) {
							elementAttributes[i++] = new HtmlForm.ElementAttribute(entry.getKey(), "CDATA",
									entry.getValue());
						}
