...
public class HTMLConverter implements ITextConverter {
	...
	@Override
	public void nodesToText(List<? extends Object> nodes, Appendable resultBuffer, IWikiModel model)
			throws IOException {
		if (nodes != null && !nodes.isEmpty()) {
			try {
				...
				if (level > Configuration.RENDERER_RECURSION_LIMIT) {
					resultBuffer.append(
							"<span class=\"error\">Error - recursion limit exceeded rendering tags in HTMLConverter#nodesToText().</span>");
					return;
				}
	...
	protected void nodeToHTML(TagNode node, Appendable resultBuffer, IWikiModel model) throws IOException {
		...
		if (HTMLTag.NEW_LINES) {
			if (name.equals("div") || name.equals("p") || name.equals("table") || name.equals("ul") || name.equals("ol")
					|| name.equals("li") || name.equals("th") || name.equals("tr") || name.equals("td")
					|| name.equals("pre")) {
				resultBuffer.append('\n');
			}
		}
	...
	private void imageThumbToHTML(TagNode imageTagNode, Appendable resultBuffer, IWikiModel model,
			Map<String, String> map, String caption, String alt, String location, String type, int pxWidth,
		...
		if (caption != null && caption.length() > 0) {
			if (alt.length() == 0) {
				resultBuffer.append(" alt=\"").append(caption).append('"').append(" title=\"").append(caption)
						.append('"');
			} else {
	...
	private void imageSimpleToHTML(TagNode imageTagNode, Appendable resultBuffer, IWikiModel model,
			Map<String, String> map, String caption, String alt, String location, String type, int pxWidth,
			int pxHeight) throws IOException {
		String href = map.get("href");
		if (href != null) {
