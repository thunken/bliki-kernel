...
/**
 * See <a href="https://www.mediawiki.org/wiki/Help:Magic_words">Help:Magic words</a> for a list of Mediawiki magic
 * words.
 */
public class MagicWord {
	...
	public enum MagicWordE {
		...
		/**
		 * The username of the user who made the most recent edit to the page, or the current user when previewing an
		 * edit.
		 */
	 ...
	 *
	 * Note: MediaWiki tolerates some variations in the case but they do not seem consistent, e.g.
	 * <code>{{CURRENTYEAR}} {{currentyear}} {{Currentyear}}</code> return the current year,
	 * <code>{{CURRENTYeAR}} {{cURRENTYEAR}}</code> don't. The following variations of {{SERVERNAME}} however all return
	 * the server name: <code>{{SERVERNAME}} {{SERVeRNAMe}} {{sERVERNAME}} {{servername}} {{Servername}}</code>
	 *
	...
	/**
	 * Determine if a template name corresponds to a magic word requiring special handling. See
	 * <a href="https://www.mediawiki.org/wiki/Help:Magic_words">Help:Magic words</a> for a list of Mediawiki magic
	 * words.
	 *
	 * @param name
	 *            the potential magic word
	 * @return <tt>true</tt> if <tt>name</tt> was a magic word, <tt>false</tt> otherwise
	 */
	 ...
	 * @param name
	 *            the potential magic word
	 * @return if <tt>name</tt> was a magic word: the corresponding {@link MagicWordE}, otherwise <tt>null</tt>
	 */
	...
	/**
	 * Process a magic word, returning the value corresponding to the magic word value. See
	 * <a href="https://www.mediawiki.org/wiki/Help:Magic_words">Help:Magic words</a> for a list of Mediawiki magic
	 * words.
	 *
	 * @param magicWord
	...
	/**
	 * Gets the sub page name of a given non-<tt>null</tt> parameter or the current model's pagename and namespace.
	 *
	 * @param parameter
	...
	/**
	 * Gets the base page name of a given non-<tt>null</tt> parameter or the current model's pagename and namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word
	 * @param model
	...
	/**
	 * Gets the full page's name of a given non-<tt>null</tt> parameter or the current model's pagename and namespace.
	 *
	 * @param parameter
	...
	private static String fullUrl(IWikiModel model, String parameter, boolean encode) {
		...
		return model.getWikiBaseURL().replace("${title}", encode ? model.encodeTitleToUrl(name, true) : name)
				.replaceAll("^https?:", "");
	}

	/**
	 * Gets the talkpage's name of a given non-<tt>null</tt> parameter or the current model's pagename and namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 * @param model
	...
	/**
	 * Gets the talkspace of a given non-<tt>null</tt> parameter or the current model's namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 * @param model
	...
	/**
	 * Gets the subject/articlespace of a given non-<tt>null</tt> parameter or the current model's namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 * @param model
	...
	/**
	 * Helper to get the namespace of either a given non-<tt>null</tt> parameter or the current model's namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 * @param model
	...
	/**
	 * Helper to get the pagename (excluding the namespace) of either a given non-<tt>null</tt> parameter or the current
	 * model's pagename.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 ...
	 *            the model being used
	 *
	 * @return the extracted pagename or <tt>null</tt> if the parameter was empty
	 */
	...
	/**
	 * Helper to get the pagename and the namespace of either a given non- <tt>null</tt> parameter or the current
	 * model's pagename and namespace.
	 *
	 * @param parameter
	 *            the parameter of the magic word (may be <tt>null</tt>)
	 * @param model
	 *            the model being used
	 *
	 * @return a 2-element array with the namespace (index 0) and the page title (index 1) or <tt>null</tt> if the
	 *         parameter was empty
	 */
	protected static String[] getPagenameHelper2(String parameter, IWikiModel model) {
		...
		return new String[] { model.getNamespace().getNamespace(model.getNamespaceName()).toString(),
				model.getPageName() };
	}
}
