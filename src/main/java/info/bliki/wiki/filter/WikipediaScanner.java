...
public class WikipediaScanner {
		...
		try {
			...
			while (true) {
				if (ch == WPList.DL_DD_CHAR) {
					if ((fScannerPosition < fSource.length - 2) && fSource[fScannerPosition] == '/'
	 ...
	 *
	 * @return the offset position behind the next closing HTML comment tag or <code>-1</code> if no tag could be found.
	 */
	 ...
	 *
	 * @return the offset position behind the &lt;/nowiki&gt; tag or <code>-1</code> if no tag could be found.
	 */
	public int indexEndOfNowiki() {
		...
		while (fScannerPosition < fSource.length - 8) {
			...
			if (ch == '<' && fSource[fScannerPosition] == '/' && fSource[fScannerPosition + 1] == 'n'
					&& fSource[fScannerPosition + 2] == 'o' && fSource[fScannerPosition + 3] == 'w'
					&& fSource[fScannerPosition + 4] == 'i' && fSource[fScannerPosition + 5] == 'k'
					&& fSource[fScannerPosition + 6] == 'i' && fSource[fScannerPosition + 7] == '>') {
	...
	/**
	 * Get the offset position behind the corresponding wiki table closing tag (i.e. <code>|}</code>). The scanner
	 * detects HTML comment tags, &lt;nowiki&gt; tags and nested wiki table tags (i.e.
	 * <code>{|... {|...  ...|} ...|}</code>).
	 *
	 * @return the offset position behind the corresponding wiki table closing tag or <code>-1</code> if no
	 *         corresponding tag could be found.
	 */
			...
			while (fScannerPosition < fSource.length) {
				...
				if (ch == '<' && fSource[fScannerPosition] == '!' && fSource[fScannerPosition + 1] == '-'
					...
					if (fScannerPosition == (-1)) {
						...
						&& fSource[fScannerPosition + 2] == 'w' && fSource[fScannerPosition + 3] == 'i'
						&& fSource[fScannerPosition + 4] == 'k' && fSource[fScannerPosition + 5] == 'i'
	 ...
	 *            the list which contains the splitted strings
	 * @param maxParts
	 *            max number of parts to split the source into (less than <tt>0</tt> for infinite number of parts,
	 *            otherwise only values greater than <tt>0</tt> allowed!)
	 * @return splitted strings
	 */
	public static List<String> splitByChar(final char splitChar, String sourceString, List<String> resultList,
			final int maxParts) {
	...
	/**
	 * Read until the end of a nested block i.e. something like <code>[[...[[  ]]...]]</code>
	 *
	 * @param sourceArray
	 * @param startCh
	 * @param endChar
	 * @param startPosition
	 * @return the position of the nested end charcters or <code>-1</code> if not found
	 */
	public static int findNestedEnd(final char[] sourceArray, final char startCh, final char endChar,
			int startPosition) {
		char ch;
		int level = 1;
	 ...
	 * @return the position of the nested end charcters or <code>-1</code> if not found
	 */
	public static int findNestedEndSingle(final char[] sourceArray, final char startCh, final char endChar,
			int startPosition) {
		char ch;
		int level = 1;
		int position = startPosition;
	...
	/**
	 * @return the template end position or -1 if there is no end. TODO: This logic needs to be improved, there are very
	 *         likely cases where this does not work as intended. Template parameters can take {} as well, e.g.
	 ...
	 * @param sourceArray
	 * @param startPosition
	 * @return an array of two integers. If <code>array[0] > 0</code> the scanner has found the end position of a
	 *         template parameter declaration. If <code>array[1] > 0</code> the scanner has found the end position of a
	 ...
	 * <p>
	 * Elements may have associated properties, called attributes, which may have values (by default, or set by authors
	 * or scripts). Attribute/value pairs appear before the final ">" of an element's start tag. Any number of (legal)
	 ...
	 * &lt;/H1&gt;
	 * </code> By default, SGML requires that all attribute values be delimited using either double quotation marks
	 * (ASCII decimal 34) or single quotation marks (ASCII decimal 39). Single quote marks can be included within the
	 * attribute value when the value is delimited by double quote marks, and vice versa. Authors may also use numeric
	 * character references to represent double quotes (&amp;#34;) and single quotes (&amp;#39;). For doublequotes
	 * authors can also use the character entity reference &amp;quot;.
	 * <p>
	 * In certain cases, authors may specify the value of an attribute without any quotation marks. The attribute value
	 * may only contain letters (a-z and A-Z), digits (0-9), hyphens (ASCII decimal 45), periods (ASCII decimal 46),
	 * underscores (ASCII decimal 95), and colons (ASCII decimal 58). We recommend using quotation marks even when it is
	 ...
	 * </ol>
	 * <p>
	 * The starting point for the various components is stored in an array of integers that match the initiation point
	 * for the states one-for-one, i.e. bookmarks[0] is where state 0 began, bookmarks[1] is where state 1 began, etc.
	 * Attributes are stored in a <code>Vector</code> having one slot for each whitespace or attribute/value pair. The
	 * first slot is for attribute name (kind of like a standalone attribute).
	 *
	 * @param start
	...
	/**
	 * Read the characters until the concatenated <i>start</i> and <i>end</i> substring is found. The end substring is
	 * matched ignoring case considerations.
	 *
	 * @param startString
	...
	/**
	 * Read the characters until no more letters are found or the given <code>testChar</code> is found. If
	 * <code>testChar</code> was found, return the offset position.
	 *
	 * @param testCh
	 *            the test character
	 * @param fromIndex
	 *            read from this offset
	 * @return <code>-1</code> if the character could not be found or no more letter character were found.
	 */
