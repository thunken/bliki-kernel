 ...
 * <a href="https://www.mediawiki.org/wiki/Manual:Parser_functions">template parser functions</a> and
...
public interface IConfiguration {
	...
	/**
	 * Add a new <a href="https://www.mediawiki.org/wiki/Extension:SyntaxHighlight_GeSHi">source code formatter</a> to
	 * the configuration
	 */
	SourceCodeFormatter addCodeFormatter(String key, SourceCodeFormatter value);

	/**
	 * Add another Interwiki link to the configuration. The value string must contain the <code>$1</code> placeholder
	 * for the used wiki article link.
	 *
	 * @param key
	 *            the prefix used in the interwiki link (i.e. [[prefix:...]] )
	 * @param value
	 *            the external link which should be generated from the interwiki link
	 */
	String addInterwikiLink(String key, String value);

	/**
	 * Add a new <a href="https://www.mediawiki.org/wiki/Interwiki">interwiki link</a> to the configuration
	 */
	ITemplateFunction addTemplateFunction(String key, ITemplateFunction value);

	/**
	 * Add a new <a href="https://www.mediawiki.org/wiki/Manual:Tag_extensions">HTML styled tag</a> to the configuration
	 */
	TagToken addTokenTag(String key, TagToken value);

	/**
	 * Get the <a href="https://www.mediawiki.org/wiki/Extension:SyntaxHighlight_GeSHi">source code formatter</a> map of
	 * built-in source code formatters
	 */
	...
	/**
	 * Get the <a href="https://www.mediawiki.org/wiki/Manual:Parser_functions">template parser functions</a> map of
	 * built-in template functions
	 */
	Map<String, ITemplateFunction> getTemplateMap();

	/**
	 * Get the <a href="https://www.mediawiki.org/wiki/Manual:Tag_extensions">HTML styled tag</a> map for built-in tags
	 */
	...
	/**
	 * Set a cache map implementation. For example based on <a href="https://jcp.org/en/jsr/detail?id=107">JSR 107</a>.
	 * Template calls which use the same parameters over and over again do lookup this cache and use the preparsed
	 * result if available.
	 *
