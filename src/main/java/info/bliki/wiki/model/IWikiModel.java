...
public interface IWikiModel extends IConfiguration {
	...
	/**
	 * When an article contains a token indicating that the article belongs to a specific category this method should be
	 * called to add that category to the output metadata. For the <code>sortKey</code> see also
	 * <a href= "https://en.wikipedia.org/wiki/Wikipedia:Categorization#Category_sorting"
	 * >Wikipedia:Categorization#Category_sorting</a>
	 *
	 * @param categoryName
	 *            The name of the category that the document belongs to.
	 * @param sortKey
	 *            The sort key for the category, or <code>null</code> if no sort key has been specified. The sort key
	 *            determines what order categories are sorted on category index pages, so a category for "John Doe"
	...
	/**
	 * When a document contains a token indicating that the document links to another Wiki topic this method should be
	 * called to add that topic link to the output metadata.
	 *
	 * @param topicName
	...
	/**
	 * When a document contains a token indicating that the document includes a Wiki template this method should be
	 * called to add that template to the output metadata.
	 *
	 * @param template
	 *            The name of the template that is being included (excluding the template namespace).
	 */
	void addTemplate(String template);

	/**
	 * When a document contains a token indicating that the document includes an other Wiki page, i.e. transclusion,
	 * this method should be called to add that page to the output metadata.
	 *
	 * This excludes transcluded template pages.
	 *
	 * @param pageName
	 *            The name of the page that is being included (including its namespace).
	 *
	 ...
	 * @param startPosition
	 *            the start index in the text where the header line of the section begins
	 * @param endPosition
	 *            the start index in the text where the header line of the section ends
	 * @return the &quot;table of content&quot; tag
	 */
	ITableOfContent appendHead(String rawHead, int headLevel, boolean noToC, int headCounter, int startPosition,
			int endPosition);

	/**
	 * Append this internal wiki image link. In the current implementation some parts of the image rendering are moved
	 * to the <code>HTMLConverter#imageNodeToText()</code> method.
	 *
	 * @param hrefImageLink
	 * @param srcImageLink
	 * @param imageFormat
	 */
	void appendInternalImageLink(String hrefImageLink, String srcImageLink, ImageFormat imageFormat);

	/**
	 * Append an internal wikilink as described in <a href="https://en.wikipedia.org/wiki/Help:Link#Wikilinks">Help
	 * Links</a>
	 *
	 * @param topic
	 * @param hashSection
	 ...
	 *            TODO
	 */
	void appendInternalLink(String topic, String hashSection, String topicDescription, String cssClass,
	 ...
	 * @param linkText
	 */
	void appendInterWikiLink(String namespace, String title, String linkText);

	/**
	 * Append an external ISBN link (starting with <code>ISBN </code>) as described in
	 * <a href="https://en.wikipedia.org/wiki/Wikipedia:ISBN">Wikipedia:ISBN</a>
	 *
	 * <br/>
	 * <br/>
	 * <b>Note:</b> The default implementation in the <code>AbstractWikiModel</code> class creates a link to
	 * <a href="https://amazon.com">Amazon.com</a>.
	 *
	 * @param isbnPureText
	 *            the pure ISBN string which contains the ISBN prefix and optional dashes in the ISBN number
	 */
	void appendISBNLink(String isbnPureText);

	/**
	 * Append an external link (starting with mailto:...) as described in
	 * <a href ="https://en.wikipedia.org/wiki/Help:Wikitext#Links">Help:Wikitext#Links </a>
	 *
	 * @param link
	 ...
	 *            the link name which is separated from the URL by a space
	 * @param withoutSquareBrackets
	 *            if <code>true</code> a mailto link with no square brackets around the link was parsed
	 */
	void appendMailtoLink(String link, String linkName, boolean withoutSquareBrackets);

	/**
	 * Check if the topic is a special namespace topic. In the <code>AbstractWikiModel</code> defaults implementation
	 * this namespace topic is parsed and checks for various namespaces, like <code>Categories</code> and
	 * <code>Interwiki</code> links.
	 *
	 * @param rawNamespaceTopic
	 *            the text between the [[...]] square brackets of a wiki link before the pipe symbol
	 * @param viewableLinkDescription
	 * @param containsNoPipe
	 *            set to <code>true</code> if the rawLinkText contained no pipe symbol.
	 * @return <code>true</code> if the topic is a special namespace topic
	 */
	boolean appendRawNamespaceLinks(String rawNamespaceTopic, String viewableLinkDescription, boolean containsNoPipe);

	/**
	 * Main entry method for parsing a raw wiki link (i.e. the text between the [[...]] square brackets). In the
	 * <code>AbstractWikiModel</code> defaults implementation this link is parsed and the various other
	 * <code>append...</code> methods of the model are called for the different cases.
	 *
	 * @param rawLinkText
	 *            the text between the [[...]] square brackets of a wiki link
	 * @param suffix
	 *            a String of lowercase letters which directly follow the link after the closing ]] brackets. Useful for
	 *            topic plurals.
	 * @return <code>true</code> if the method used the suffix for rendering the wiki link. If <code>false</code> the
	 *         parser should append the suffix as normal text after the wiki link (i.e. in the case of an image of file
	 *         wiki link).
	 */
	...
	/**
	 * Append the user signature to the writer (i.e. '~~~', '~~~~' or '~~~~'). This method is used in the first pass of
	 * parsing the wiki text (together with template parsing). You can append a [[User:xxxx]] string according to your
	 * model.
	 *
	 *
	 * @param writer
	 ...
	 * @throws IOException
	 */
	void appendSignature(Appendable writer, int numberOfTildes) throws IOException;
	...
	/**
	 * Decrement the current recursion level of the parser. The recursion level is used to prevent infinite nesting of
	 * templates, tables, lists and other parser objects.
	 *
	...
	/**
	 * Decrement the current template recursion level of the temlate parser. The recursion level is used to prevent
	 * infinite nesting of templates, tables, lists and other parser objects.
	 *
	 * @return the decremented recursion level
	 */
	...
	/**
	 * Encode the <i>wiki links title</i> into a URL for HTML hyperlinks (i.e. create the <i>href</i> attribute
	 * representation for the <i>a</i> tag). This method uses the '.' character to encode special characters. To get the
	 * <a href="https://meta.wikimedia.org/wiki/Help:Page_name#Case-sensitivity">behavior of the MediaWiki software</a>,
	 * which is configured to convert the first letter to upper case, the <code>firstCharacterAsUpperCase</code>
	 * parameters must be set to <code>true</code>. For an example encoding routine see:
	...
	/**
	 * Get the current time stamp. This is the value for the magic word &quot;CURRENTTIMESTAMP&quot;.
	 *
	 * This method typically returns the value <code>new Date(System.currentTimeMillis());</code>.
	 *
	 * @return a date value
	 */
	...
	/**
	 * Return a URL string which contains a &quot;${image}&quot; variable, which will be replaced by the image name, to
	...
	/**
	 * Get the <a href="https://www.mediawiki.org/wiki/Interwiki">interwiki links</a> map for converting interwiki links
	 * into external URLs.
	 *
	 * Example: maps the interwiki shortcut &quot;de&quot; to &quot;https://de.wikipedia.org/wiki/$1&quot;
	 */
	 ...
	 * <code>getNode(fWikiModel.stackSize() - 2)</code> returns the node before the node at top of the stack.
	 *
	 * @param offset
	 ...
	 *            if the namespace is the <b>Template</b> namespace, the current template parameters are stored as
	 *            <code>String</code>s in this map
	 *
	 * @return <code>null</code> if no content was found
	 * @see AbstractParser#parsePageName(IWikiModel, String, INamespaceValue, boolean)
	 */
	@Nullable
	String getRawWikiContent(ParsedPageName templateName, Map<String, String> templateParameters)
			throws WikiModelContentException;

	/**
	 * Get the current recursion level of the parser. The recursion level is used to prevent infinite nesting of
	 * templates, tables, lists and other parser objects.
	 *
	 ...
	 *
	 * @return the raw string between the wikilink tags <code>[[ ]]</code> or <code>null</code> if no redirect exists
	 */
	 ...
	 *
	 * @return the list of SemanticAttributes or <code>null</code> if no SemanticAttribute exists
	 */
	 ...
	 * See <a href="https://en.wikipedia.org/wiki/Semantic_MediaWiki">Semantic MediaWiki</a> for more information.
	 *
	 * @return the list of SemanticRelations or <code>null</code> if no SemanticRelation exists
	 */
	...
	/**
	 * Get a template parser function (i.e. <code>{{ #if: ... }}</code> ) implementation.
	 *
	 * @param name
	 *            the name of the function without the &quot;#&quot; and &quot;:&quot; delimiters
	 * @return the parser function or <code>null</code> if no function is available for the given name
	 */
	ITemplateFunction getTemplateFunction(String name);

	/**
	 * Return a URL string which contains, a &quot;${title}&quot; variable which will be replaced by the topic title, to
	 * create links edit pages of wiki topics.
	 *
	...
	/**
	 * Increment the current recursion level of the parser. The recursion level is used to prevent infinite nesting of
	 * templates, tables, lists and other parser objects.
	 *
	 * @return the current recursion level counter
	 */
	...
	/**
	 * Increment the current recursion level of the template parser. The recursion level is used to prevent infinite
	...
	/**
	 * Checks if <a href="https://en.wikipedia.org/wiki/CamelCase">CamelCase</a> words should also be used as wiki
	 * links.
	 *
	 *
	 * @return <code>true</code> if CamelCase words should also be used as wiki links
	 */
	 ...
	 *
	 * @return <code>true</code> if the rendering of the &quot;table of contents&quot; is disabled globally.
	 */
	boolean isNoToc();
	 ...
	 *
	 * @param namespace
	 * @return <code>true</code> if the namespace is a interwiki namespace (i.e. prefix).
	 */
	boolean isInterWiki(String namespace);

	/**
	 * If the <code>&lt;math&gt;</code> tag should be rendered for the
	 * <a href="https://www.mathtran.org">www.mathtran.org</a> service, then return <code>true</code>.
	 *
	 * @return <code>true</code> the <code>&lt;math&gt;</code> tag should be rendered fro mathtran.org.
	 */
	...
	/**
	 * Allow the parsing of semantic mediawiki (SMW) links. See
	 * <a href="https://en.wikipedia.org/wiki/Semantic_MediaWiki">Semantic MediaWiki</a> for more information.
	 *
	 * @param namespace
	 * @return <code>true</code> if parsing of semantic mediawiki (SMW) links is enabled
	 */
	...
	/**
	 * Check if the scheme-specific part for a given top level URI (Uniform Resource Identifier) scheme name is valid in
	 * this model.
	 *
	 * See <a href="https://en.wikipedia.org/wiki/URI_scheme">URI scheme</a>
	 *
	 ...
	 *            the URI (Uniform Resource Identifier) scheme part following the top level scheme name and the colon
	 *            character ":"
	 * @return <code>true</code> if the specified URI scheme is valid.
	...
	/**
	 * Parse a behavior switch (i.e. an identifier with two leading and trailing underscores &quot;__&quot; like for
	 * example <code>__NOEDITSECTION__</code> ). See
	 * <a href ="https://www.mediawiki.org/wiki/Help:Magic_words#Behavior_switches">Help
	 * :Magic_words#Behavior_switches</a>.
	 *
	 * @param identifier
	 *            the identifier without the leading and trailing underscores (&quot;__&quot;)
	 ...
	 * <br/>
	 * See <a href="https://en.wikipedia.org/wiki/Image_markup">Image markup</a> and see
	 * <a href="https://www.mediawiki.org/wiki/Help:Images">Help:Images</a>.
	 *
	 * @param imageNamespace
	 *            the image namespace
	 * @param rawImageLink
	 *            the raw image link text without the surrounding <code>[[...]]</code>
	 */
	void parseInternalImageLink(String imageNamespace, String rawImageLink);
	 ...
	 *            if <code>true</code> parse only the signature wiki texts, no templates and wiki comment parsing
	 * @return
	 */
	 ...
	 *
	 * @param converter
	 *            a text converter. <b>Note</b> the converter may be <code>null</code>, if you only would like to
	 *            analyze the raw wiki text and don't need to convert. This speeds up the parsing process.
	 * @param rawWikiText
	 *            a raw wiki text
	 */
	 ...
	 * @param templateTopic
	 *            if <code>true</code>, render the wiki text as if a template topic will be displayed directly,
	 *            otherwise render the text as if a common wiki topic will be displayed.
	 * @param parseTemplates
	 *            parses the template expansion step (parses include, onlyinclude, includeonly etc)
	 */
	void render(ITextConverter converter, String rawWikiText, Appendable buffer, boolean templateTopic,
	 ...
	 *            if <code>true</code>, render the wiki text as if a template topic will be displayed directly,
	 *            otherwise render the text as if a common wiki topic will be displayed.
	 * @return <code>null</code> if an IOException occurs or <code>converter==null</code>
	 */
	...
	/**
	 * Render the raw Wikipedia text into an HTML string and use the default PDFConverter. The resulting XHTML could be
	 * used as input for the Flying Saucer PDF renderer
	 *
	 * @param rawWikiText
	...
	/**
	 * Set the "lower-case" namespace name of the article rendered with this model. This name will be converted with the
	 * Namespace#getNamespace() method to a string in the current Locale.
	 *
	...
	/**
	 * Set to <code>true</code> if the rendering of the &quot;table of contents&quot; should be disabled globally.
	 *
	 * @param disableToc
	 *            set to <code>true</code> if the rendering of the &quot;table of contents&quot; should be disabled
	 *            globally.
	 */
	 ...
	 *
	 * @return a magic word object (e.g. {@link info.bliki.wiki.filter.MagicWord.MagicWordE} in case
	 *         {@link info.bliki.wiki.filter.MagicWord} is used) or <tt>null</tt> if this is no valid magic word
	 */
	@Nullable
	Object getMagicWord(String name);

	/**
	 * Splits the given full title into its namespace and page title components and normalises both components using
	 * {@link Encoder#normaliseTitle(String, boolean, char)} keeping underscores.
	 *
	 * @param fullTitle
	 *            the (full) title including a namespace (if present)
	 *
	 * @return a 2-element array with the namespace (index 0) and the page title (index 1)
	 */
