...
public interface INamespace {
	...
	public enum NamespaceCode {
		...
		/**
		 * Information about the wiki. Prefix is the same as $wgSitename of the PHP installation.
		 */
		 ...
		 */
		private static final EnumSet<NamespaceCode> wgNamespacesWithSubpages = EnumSet.of(TALK_NAMESPACE_KEY,
				USER_NAMESPACE_KEY, USER_TALK_NAMESPACE_KEY, PROJECT_NAMESPACE_KEY, PROJECT_TALK_NAMESPACE_KEY,
				FILE_TALK_NAMESPACE_KEY, MEDIAWIKI_NAMESPACE_KEY, MEDIAWIKI_TALK_NAMESPACE_KEY,
				TEMPLATE_TALK_NAMESPACE_KEY, HELP_NAMESPACE_KEY, HELP_TALK_NAMESPACE_KEY, CATEGORY_TALK_NAMESPACE_KEY);
	...
	public interface INamespaceValue {
		...
		/**
		 * Re-sets the texts used for this namespace. The first will be the primary text.
		 *
		 * @param aliases
		 ...
		 *
		 * @return the primary text for the namespace, i.e. the first value of {@link #getTexts()}.
		 */
		...
		/**
		 * Get the associated namespace. For talk namespaces, returns the subject (non-talk) namespace For subject
		 * (non-talk) namespaces, returns the talk namespace
		 *
		...
		/**
		 * Prepends the namespace to the given pagename and returns the full name with a separation character between a
		 * (non-empty) namespace and the page name.
		 *
		 * @param pageName
		 ...
		 *            the code
		 *
		 * @return <tt>true</tt> if the namespace is of the given code, <tt>false</tt> otherwise
		 */
		...
		/**
		 * Does the namespace (potentially) have different aliases for different genders. Not all languages make a
		 * distinction here.
		 */
	 ...
	 *            the code
	 *
	 * @return <tt>true</tt> if the given namespace is of the given code, <tt>false</tt> otherwise
	 */
	 ...
	 * @param namespace
	 *            the namespace
	 * @return the talk namespace for the given namespace or <tt>null</tt> if the namespace is invalid or there is no
	 *         talk namespace
	 *
	 ...
	 * {@link Encoder#normaliseTitle(String, boolean, char, boolean)}. Assumes <tt>underScoreIsWhitespace</tt>, uses a
	 * space as <tt>whiteSpaceChar</tt> and capitalises the first character.
	 *
	 * @param fullTitle
	 *            the (full) title including a namespace (if present)
	 *
	 * @return a 2-element array with the raw namespace string (index 0) and the page title (index 1)
	 ...
	 * @return a 2-element array with the raw namespace string (index 0) and the page title (index 1)
	 */
	public abstract String[] splitNsTitle(String fullTitle, boolean underScoreIsWhitespace, char whiteSpaceChar,
