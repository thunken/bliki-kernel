...
public class GroovyCodeFilter extends AbstractCPPBasedCodeFilter implements SourceCodeFormatter {
	...
	private static final String[] KEYWORDS = { "any", "as", "class", "abstract", "break", "byvalue", "case", "cast",
			"catch", "const", "continue", "def", "default", "do", "else", "extends", "false", "final", "finally", "for",
			"future", "generic", "goto", "if", "implements", "import", "in", "inner", "instanceof", "interface",
			"mixin", "native", "new", "null", "operator", "outer", "package", "private", "property", "protected",
			"public", "rest", "return", "static", "super", "switch", "synchronized", "test", "this", "threadsafe",
			"throw", "throws", "transient", "true", "try", "using", "var", "volatile", "while", "assert", "enum",

			"boolean", "char", "byte", "short", "int", "long", "float", "double", "void" };

	private static final String[] OBJECT_WORDS = { "Boolean", "Byte", "Character", "Class", "ClassLoader", "Cloneable",
			"Compiler", "Double", "Float", "Integer", "Long", "Math", "Number", "Object", "Process", "Runnable",
		...
		try {
			while (true) {
				...
				if ((currentChar >= 'A' && currentChar <= 'Z') || (currentChar == '_')
					...
					continue;
				} else if (currentChar == '\'' && currentPosition < input.length() - 1
					...
					while (currentPosition < input.length()) {
						...
						if (currentChar == '\'' && currentPosition < input.length() - 1
								&& source[currentPosition] == '\'' && source[currentPosition + 1] == '\'') {
							break;
						}
					}
