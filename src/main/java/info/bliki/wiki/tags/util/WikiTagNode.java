...
/**
 * WikiTagNode represents a generic tag. If no scanner is registered for a given tag name, this is what you get. This is
 * also the base class for all tags created by the parser.
 */
public class WikiTagNode extends AbstractNode {
	...
	/**
	 * The tag attributes. Objects of type {@link Attribute}. The first element is the tag name, subsequent elements
	 * being either whitespace or real attributes.
	 */
	 ...
	 * @return The value associated with the attribute or null if it does not exist, or is a stand-alone or
	 */
	...
	/**
	 * Set an attribute. This replaces an attribute of the same name. To set the zeroth attribute (the tag name), use
	 * setTagName().
	 *
	 * @param attribute
	 *            The attribute to set.
	 ...
	 *
	 * @return Returns the list of {@link Attribute Attributes} in the tag. The first element is the tag name,
	 *         subsequent elements being either whitespace or real attributes.
	 */
	 ...
	 * <p>
	 * <em> Note: This value is converted to uppercase and does not begin with "/" if it is an end tag. Nor does it end
	 * with a slash in the case of an XML type tag. To get at the original text of the tag name use
	 * {@link #getRawTagName getRawTagName()}. The conversion to uppercase is performed with an ENGLISH locale. </em>
	 *
	 * @return The tag name.
	 */
	 ...
	 * Return the name of this tag.
	 *
	 * @return The tag name or null if this tag contains nothing or only whitespace.
	 */
	...
	/**
	 * Set the name of this tag. This creates or replaces the first attribute of the tag (the zeroth element of the
	 * attribute vector).
	 *
	 * @param name
	 *            The tag name.
	...
	/**
	 * Sets the attributes. NOTE: Values of the extended hashtable are two element arrays of String, with the first
	 * element being the original name (not uppercased), and the second element being the value.
	 *
	 * @param attribs
	 ...
	 *
	 * @return An empty string (tag contents do not display in a browser). If you want this tags HTML equivalent, use
	 *         {@link #toHtml toHtml()}.
	 */
	@Override
	 ...
	 *
	 * @param verbatim
	 *            If <code>true</code> return as close to the original page text as possible.
	...
	/**
	 * Return the set of tag names that cause this tag to finish. These are the normal (non end tags) that if
	 * encountered while scanning (a composite tag) will cause the generation of a virtual tag. Since this a a
	...
	/**
	 * Return the set of end tag names that cause this tag to finish. These are the end tags that if encountered while
	 * scanning (a composite tag) will cause the generation of a virtual tag. Since this a a non-composite tag, it has
	 * no end tag enders.
	 *
