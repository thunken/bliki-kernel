 ...
 * in MediaWiki SVN</a> or<a href="https://en.wikipedia.org/wiki/Help:Calculation">Wikipedia - Help:Calculation</a>.
 */
@Slf4j
public class Expr extends AbstractTemplateFunction {
	...
	public String parseFunction(final List<String> list, final IWikiModel model, final char[] src, final int beginIndex,
			final int endIndex, final boolean isSubst) throws IOException {
		if (list.size() > 0) {
			...
			if (!isSubst) {
				final StringBuilder conditionBuffer = new StringBuilder(expression.length());
			...
			try {
				...
				final double d = engine.evaluate(expression);
				return getWikiNumberFormat(d, model);
			} catch (final Exception e) {
				if (Configuration.DEBUG) {
					log.debug("#expr error: " + expression);
				}
	...
	public static String getWikiNumberFormat(final double d, final IWikiModel model) {
		final double dInt = Math.rint(d);
		// if (dInt == d) {
