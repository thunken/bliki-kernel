...
public class Fullurl extends AbstractTemplateFunction {
	...
	public String parseFunction(List<String> list, IWikiModel model, char[] src, int beginIndex, int endIndex,
			boolean isSubst) throws UnsupportedEncodingException {
		if (list.size() > 0) {
			String arg0 = isSubst ? list.get(0) : parseTrim(list.get(0), model);
			if (arg0.length() > 0) {
				if (list.size() == 1) {
					String baseUrl = model.getWikiBaseURL();
					baseUrl = baseUrl.replace("${title}",
							URLEncoder.encode(Character.toUpperCase(arg0.charAt(0)) + "", UTF_8.name())
				...
				baseEditUrl = baseEditUrl.replace("${title}",
						URLEncoder.encode(Character.toUpperCase(arg0.charAt(0)) + "", UTF_8.name())
								+ URLEncoder.encode(arg0.substring(1), UTF_8.name()));
