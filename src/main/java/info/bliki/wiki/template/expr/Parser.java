 ...
 *
 * See <a href="https://en.wikipedia.org/wiki/Operator-precedence_parser">Operator -precedence parser</a> for the idea,
...
public class Parser extends Scanner {
		...
		while (true) {
			...
			if (operLookahead instanceof InfixOperator) {
				...
				if (binOper.getPrecedence() > min_precedence) {
					...
					continue;
				} else if ((binOper.getPrecedence() == min_precedence)
						&& (binOper.getGrouping() == InfixOperator.RIGHT_ASSOCIATIVE)) {
					rhs = parseOperators(rhs, operLookahead.getPrecedence());
					continue;
				}
			} else {
	...
	/**
	 * See <a href="https://en.wikipedia.org/wiki/Operator-precedence_parser">Operator -precedence parser</a> for the
