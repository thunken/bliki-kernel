...
public class GenerateOperatorArrays {
	...
	/**
	 * Utility for generating source codes for the
	 * <code>ASTNodeFactory's HEADER_STRINGS, OPERATOR_STRINGS, OPERATORS</code> arrays from an operator's text file
	 * description
	 *
	 * @param args
	 *            if <code>args.length==0</code> take the default <code>/opertors.txt</code> file for generating the
	 *            arrays; if <code>args.length>=1</code> the <code>arg[0]</code> parameters should contain the complete
	 *            filename of the operator's description file
	 */
		...
		try {
			...
			while (i3.hasNext()) {
				...
				if (oper instanceof DivideOperator) {
					...
					System.out.println("    new DivideOperator(\"" + iOper.getOperatorString() + "\", \""
					...
					System.out.println("    new InfixOperator(\"" + iOper.getOperatorString() + "\", \""
							+ iOper.getFunctionName() + "\", " + iOper.getPrecedence() + ", " + grouping + "),");
				} else if (oper instanceof PostfixOperator) {
					System.out.println("    new PostfixOperator(\"" + oper.getOperatorString() + "\", \""
							+ oper.getFunctionName() + "\", " + oper.getPrecedence() + "),");
				} else if (oper instanceof PreMinusOperator) {
					System.out.println("    new PreMinusOperator(\"" + oper.getOperatorString() + "\", \""
							+ oper.getFunctionName() + "\", " + oper.getPrecedence() + "),");
				} else if (oper instanceof PrePlusOperator) {
					System.out.println("    new PrePlusOperator(\"" + oper.getOperatorString() + "\", \""
							+ oper.getFunctionName() + "\", " + oper.getPrecedence() + "),");
				} else if (oper instanceof PrefixOperator) {
					System.out.println("    new PrefixOperator(\"" + oper.getOperatorString() + "\", \""
							+ oper.getFunctionName() + "\", " + oper.getPrecedence() + "),");
				}
	...
	public static void generateOperatorTable(final InputStream is, final HashMap<String, Operator> operatorMap,
			final HashMap<String, ArrayList<Operator>> operatorTokenStartSet) {
		String record = null;
