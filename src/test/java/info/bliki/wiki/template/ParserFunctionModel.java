...
/**
 * Wiki model implementation which allows some special JUnit tests for template parser functions
 *
 */
public class ParserFunctionModel extends WikiModel {
	...
	// "<includeonly>{{#ifeq:{{{sortable|}}}|on|<span
	// style=\"display:none\">{{padleft:{{{1}}}|16|0}}</span>}}{{convert/{{{2}}}|{{{1}}}|{{#ifeq:{{#expr:{{{3|0}}}*0}}|0|0}}|{{{3|}}}|{{{4|}}}|{{{5|}}}|{{{6|}}}|r={{#ifeq:{{{sp}}}|us|er|re}}|d=L{{{lk|off}}}A{{{abbr|off}}}D{{{disp|b}}}S{{{adj|{{{sing|off}}}}}}|s={{{sigfig|}}}}}</includeonly><noinclude>{{Documentation}}</noinclude>"
	// ;
